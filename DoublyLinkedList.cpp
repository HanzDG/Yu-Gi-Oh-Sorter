...
void DoublyLinkedList<T>::swap(int i, int j) {
  Node<T> *head = first_;
  if(head == NULL || head->getNext() == NULL || i == j) {
    return;
  }
  ...
  // Node<T>* temp_one_previous= node1->getPrevious();
  // Node<T>* temp_one_next= node1->getNext();
  // Node<T>* temp_two_previous= node2->getPrevious();
  // Node<T>* temp_two_next= node2->getNext();


  // node2->setPrevious(temp_one_previous);
  // // temp_one_previous->setNext(node2);
  // node2->setNext(temp_one_next);
  // // temp_one_next->setPrevious(node2);

  // node1->setPrevious(temp_two_previous);
  // // temp_two_previous->setNext(node1);
  // node1->setNext(temp_two_next);
  // // temp_two_next->setPrevious(node2);
